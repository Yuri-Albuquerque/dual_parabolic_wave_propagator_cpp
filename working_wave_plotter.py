...
def create_wave_plots():
    ...
    for i, (wave_data, time_step) in enumerate(zip(results.wave_data, results.time_steps)):
        ...
        ax.plot(x_para[mask_major], y_major[mask_major], 'k-', linewidth=2, alpha=0.8, label='Major Parabola')
        
        # Minor parabola (y = x²/100 - 25)
        y_minor = x_para**2 / 100 - 25
        mask_minor = (y_minor >= -300) & (y_minor <= 300)
        ax.plot(x_para[mask_minor], y_minor[mask_minor], 'k--', linewidth=2, alpha=0.8, label='Minor Parabola')
        
        # Mark focus points
        ax.plot(0, 100, 'ro', markersize=8, label='Major Focus')
        ax.plot(0, -25, 'bo', markersize=8, label='Minor Focus')
